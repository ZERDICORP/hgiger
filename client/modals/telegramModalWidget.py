...
class TelegramModalWidget(ModalWidget):
	def __init__(self, parent, ui):
		...
		self.setMinimumWidth(self.ui.WProperties[0][0] / 100 * 50)

		self.parent.setChild(self, alignment=Qt.AlignCenter)

	def cloningDatabase(self):
		self.loader.toggle()
		self.ui.manager.inThread(lambda: self.ui.manager.hgigerbot.cloningDatabase(
			dbPath=self.ui.manager.basePath, 
			hintsCount=self.ui.manager.hintsCount(),
			cloningFinished=self.cloningFinished,
			cloningError=self.cloningError))
