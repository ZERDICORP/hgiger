...
from client.modals.attachedImagesModalWidget import AttachedImagesModalWidget
...
class HintPage(PageWidget):
	...
	def afterInit(self):
		self.setAttachedImagesModal()
		self.tabWidget.currentChanged.connect(lambda index: 
			self.ui.history.updateTabIndex(self, index))

	def setAttachedImagesModal(self):
		self.attachedImagesModal = AttachedImagesModalWidget(parent=self.ui.createSmokeWidget(), pageWidget=self, ui=self.ui, images=self.hint["images"])
		if self.getHistory()["attachedImages"]:
			self.attachedImagesModal.toggle()
		QShortcut(QKeySequence("Ctrl+I"), self).activated.connect(self.attachedImagesModal.toggle)
	...
	def contextMenuEvent(self, event):
		contextMenu = ContextMenuWidget(self, self.ui.RobotoLight, [
			...
			False],
			[QAction(QIcon(assetsPath.image), "Attached images", self), self.attachedImagesModal.toggle, 
			False],
		])
